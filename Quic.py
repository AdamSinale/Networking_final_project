...
class QuicPacket:
    ...
    def __str__(self):
        payload_str = "[" + ", ".join(str(frame) for frame in self.payload) + "]"
        return f"QuicPacket(header={self.header}, payload={payload_str})"
...
class QuicHeader:
    ...
    def __str__(self):
        return f"QuicHeader(flags={self.flags}, packet_number={self.packet_number}, connection_id={print(self.connection_id)})"
...
class QuicHeaderFlags:
    ...
    def __str__(self):
        return f"QuicHeaderFlags(ack={self.ack}, syn={self.syn}, data={self.data}, fin={self.fin})"
...
class QuicFrame:
    ...
    def serialize(self):
        ...
        offset_bytes = self.offset.to_bytes(2, byteorder='big')  # Assuming offset is a 16-bit integer
        length_bytes = self.length.to_bytes(2, byteorder='big')  # Assuming length is a 16-bit integer
        return stream_id_bytes + offset_bytes + length_bytes + self.data

    @classmethod
    def deserialize(cls, serialized_data):
        stream_id = int.from_bytes(serialized_data[:4], byteorder='big')  # Deserialize stream_id
        offset = int.from_bytes(serialized_data[4:6], byteorder='big')  # Deserialize offset
        length = int.from_bytes(serialized_data[6:8], byteorder='big')  # Deserialize length
        data = serialized_data[8:]  # Extract data
        return cls(stream_id, offset, length, data)
    
    def __str__(self):
        return f"QuicFrame(stream_id={self.stream_id}, offset={self.offset}, length={self.length}, data={self.data})"
...
payload = [QuicFrame(1,0, 5, b"Hello"), QuicFrame(1,0, 4, b"Sup?")]
quic = QuicPacket(header=header, payload=payload)
print(quic)
